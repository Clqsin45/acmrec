...
void init(LL *t, LL *tt, int N, int k, int p)
{
    int i;
    for(i = 1; i <= N; i++)
    {
...
void calx(int len)
{
    ...
    for(i = 1; i <= n; i++)
    {
        ...
        for(j = 1; j < len; j++)
        {
            ...
            gx[i][j] = (gx[i][j - 1] * c + (LL)ch[i][j]) % HB;
            //printf("%d  %d  %lld\n",i, j, fx[i][j]);
        }
        for(; j <= m; j++)
        {
            ...
            fx[i][j] = (fx[i][j] + HA) % HA;
            gx[i][j] = (gx[i][j - 1] * c + (LL)ch[i][j] - dirxx[len] * (LL)ch[i][j - len]) % HB;
            gx[i][j] = (gx[i][j] + HB) % HB;
            //printf("%d  %d  %lld    %lld\n",i, j, fx[i][j],gx[i][j]);
        }
    }
...
bool app(int i, int j, int no, int noo)
{
    if(va[no].val == noo)
    {
        ans.a = va[no].loc / 1000;   ans.aa = i; ans.b = va[no].loc % 1000;   ans.bb = j;
        return true;
    }
    va[no].val = noo;
    va[no].loc = i * 1000 + j;
   ...
   // ty = m - len + 1;   tx = n - len + 1;
    for(j = len; j <= m; j ++)
    {
        ...
        for(; i <= n; i++)
        {
            ...
            fy[i][j] = (fy[i][j] + HA) % HA;
            gy[i][j] = (gy[i - 1][j] * d + gx[i][j] - gx[i - len][j] * diryy[len]) % HB;
            gy[i][j] = (gy[i][j] + HB) % HB;
            //printf("!!%d  %d  %lld    %lld\n",i, j, fy[i][j], gy[i][j]);
...
int main(void)
{
   ...
   //freopen("o.txt", "w", stdout);
    ...
    init(dirx, dirxx, m, x, c);   init(diry, diryy, n, y, d);
    for(i = 1; i <= n; i++)
    {
        scanf("%s", ch[i] + 1);
